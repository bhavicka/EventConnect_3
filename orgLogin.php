...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM organizers WHERE organization = '$input_username'";
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($input_password, $hashed_password)) {
            ...
            header("Location: OrgDashboard.php"); // Redirect to dashboard.php after successful login
            exit();
        } else {
...
<body>
    <div class="container">
        <h2>Organizer Login to EventConnect</h2>
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            <input type="text" name="username" placeholder="Username" required>
            <input type="password" name="password" placeholder="Password" required>
            <input type="submit" value="Login">
        </form>
        <a href="regOrganizer.html" class="back-link">not registered yet?</a>
    </div>
</body>
<footer>
